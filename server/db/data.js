...
const qFormat = (string) => {
  // let chopped = string.slice(0, -1);
  const decap = (match, offset, string) => {
    return match.toUpperCase();
  };
  return string.replace(/^[a-z]/, decap);
  // return chopped.toLowerCase();
};
const authorGen = faker.fake("{{name.firstName}} {{name.lastName}} of Yarpmenistan");

//TextBanks
const qStems = ["Does it have ", "Is it ", "Does it ", "Where ", "How does "];

const lotr = ["Frodo", "Bilbo", "Gimli", "Legolas", "Aragorn", "Gollum", "Yarp", authorGen];
const quidditch = ["Harry P", "Hermione G", "Ron W", "Snape", "Dumbledork", authorGen];
const spells = ["Merlin", "Gandalf", "Saruman", "Thizzard", "Raiden", authorGen];
const accessories = ["CosplayJunkie99", "CosplayAllDayBaby", "Cosplaya", "DressUpTime", authorGen];
const wands = ["Merlin", "Gandalf", "Saruman", "Thizzard", "Voldemort", authorGen];
const rarities = ["RareLord", "CollectorOfObscurities", "RareStyle", "Yarp", authorGen];
const food = ["FoodLord", "ILuvMagicalEdibles", "Yarpin93", authorGen];
const furniture = ["OrnateLover", "FounderAndCEOofMythicalFurnishings", "Yarp", authorGen];
const lamps = ["Genie", "Aladdin", "StillWishin", "Snarfin99", authorGen];

//generate random qa-docs
const genQA = (catArr) => {
  let qaDoc = {
    "Qstn": (qStems[randomizerFloor(5)] + faker.fake("{{company.catchPhrase}}").toLowerCase() + "?"),
    "Ans": qFormat(faker.fake("{{company.bs}} {{company.bsBuzz}} {{company.bsNoun}} {{company.bsAdjective}} {{company.bsBuzz}} {{company.bsNoun}}.")),
    "Author": catArr[randomizerFloor(catArr.length)],
    "Votes": randomizer(16),
...
const addQA = (doc => {
  ...
  let catArr = cat === "lotr" ? lotr : cat === "quidditch" ? quidditch : cat === "accessories" ? accessories : cat === "spells" ? spells : cat === "wands" ? wands : cat === "rarities" ? rarities : cat === "food" ? food : cat === "furniture" ? furniture : cat === "lamps" ? lamps : lotr;
  doc["QA"] = [];
  let qaListLength = randomizer(13);
  for (let i = 0; i < qaListLength; i++) {
    doc["QA"].push(genQA(catArr));
  }
});
...
//Test
console.log('Test FAke: ' + faker.fake("{{name.firstName}} {{name.lastName}} of Yarpistan"));
console.log('Single Product document: ' + JSON.stringify(data[104]));
